...
def vote_page():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if user.team_id != current_app.config['VOTE_TEAM']:
            ...
            abort(403)
        vote = Vote.query.filter_by(voter=current_user.get_id(), lecturer=form.lecturer.data).first()
        if not vote:
            vote = Vote()
        with db.auto_commit():
            ...
            db.session.add(vote)
    return render_template('vote.html', lecturers=get_team_members(current_app.config['LECTURE_TEAM']), form=form)
...
def score():
    res = db.session.query(User.name.label('name'),
                           ...
                           func.avg(Vote.voice).label('voice')).join(
        Vote, Vote.lecturer == User.username).group_by(Vote.lecturer)
...
def vote_status():
    ...
    for voter in get_team_members(current_app.config['VOTE_TEAM']):
        votes = [i.lecturer for i in Vote.query.filter_by(voter=voter.username)]
        voters.append(
            {"name": voter.name, "votes": [True if lecturer.username in votes else False for lecturer in lecturers]})
